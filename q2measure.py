...
positions, measurements = simulate(init_position = initial_position, init_velocity=initial_velocity, control_input = np.array([0,0]), delta_time = 1, Rt =  np.diag([1.0, 1.0, 0.01, 0.01]), Qt = np.eye(2) * (stdev**2), T_step = 200)
...
def Kalman_filter(mu_t_1, sig_t_1, u_t, z_t, del_t = 1.0):
    ...
    sig_t_dash = np.matmul(A_t, np.matmul(sig_t_1, A_t.T)) + R_t
    
    Q_t = np.eye(2)*(stdev**2)

    interm = np.linalg.inv(np.matmul(C_t, np.matmul(sig_t_dash, C_t.T)) + Q_t)
...
ax.plot(measurements[:, 0], measurements[:, 1], 'g')
#plt.show()
ax.plot(estimated_positions[:, 0], estimated_positions[:, 1], 'b')
plt.show()
...
fig, ax = plt.subplots()
ax.plot(estimated_positions[:,0], estimated_positions[:, 1], 'b')
for i, s in enumerate(uncertainties):
    s_ = s[:2, :2] #sigma_xx
    confidence_ellipse(s_, estimated_positions[i], ax, n_std=5.0, edgecolor='yellow')
ax.plot(positions[:,0], positions[:,1], 'r')
plt.plot()
plt.show()